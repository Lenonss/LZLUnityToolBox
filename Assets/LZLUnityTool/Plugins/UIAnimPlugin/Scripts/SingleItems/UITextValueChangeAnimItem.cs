...
namespace LZLUnityTool.Plugins.UIAnimPlugin
{
    public class UITextValueChangeAnimItem : MonoBehaviour
    {
        ...
        private Coroutine valueChangeCoro;
        private int curStartValue = 0;
        private int curTargetValue = 0;
        private void Awake()
        {
            //shakeScale
        ...
        {
            curStartValue = startValueP;
            curTargetValue = targetValueP;
            //对于相等的情况
            if (curStartValue == curTargetValue && CheckTextExist())
            {
                text.text = curStartValue.ToString();
                if (useChangeShake)
                {
                    PlayShakeScale();
                }
        ...
        public void ValueChange_Fixed(int startValueP,int targetValueP,float unitDurationP,bool ignoreScale = true)
        {
            ...
            if (CheckTextExist())
            {
                valueChangeCoro = StartCoroutine(IEValueChange_Fixed(startValueP, targetValueP, unitDurationP, ignoreScale));
            }
        }
        ...
        public void ValueChange_Linear(int startValueP,int targetValueP,bool ignoreScale = true)
        {
            ...
            if (CheckTextExist())
            {
               valueChangeCoro = StartCoroutine(IEValueChange_Linear(startValueP, targetValueP, ignoreScale));
            }
        }
        ...
        public void ValueChange_Cruve(int startValueP,int targetValueP,AnimationCurve cruve,bool ignoreScale = true)
        {
            ...
            if (CheckTextExist())
            {
               valueChangeCoro = StartCoroutine(IEValueChange_Cruve(startValueP, targetValueP,cruve, ignoreScale));
            }
        }
        ...
        {
            //已经达到目标值
            if (curStartValue == curTargetValue)
            {
                return;
            }

            //当前没有变换动画携程
            if (valueChangeCoro == null)
            {
                return;
            }
            
            StopChangeAnimCoro();
            text.text = curTargetValue.ToString();
        }

        /// <summary>
        /// 停止数值变化携程
        /// </summary>
        private void StopChangeAnimCoro()
        {
            if (valueChangeCoro!=null)
            {
                StopCoroutine(valueChangeCoro);
                valueChangeCoro = null;
            }
        }
